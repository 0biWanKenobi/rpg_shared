...
<script lang="ts">
    ...
    let rows: HTMLCollectionOf<Element> = $state(new HTMLCollection());
    let viewport = $state<Element|undefined>();
    let contents = $state<Element|undefined>();
    ...
    onMount(() => {
        rows = contents?.getElementsByClassName("svelcomlib-virtual-list-row") ?? new HTMLCollection();
        mounted = true;
    });
</script>
